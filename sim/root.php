...
class penjualan
{
	...
	function tambah_barang($nama_barang,$stok,$harga_beli,$harga_jual,$id_kategori){
		...
		if ($query->num_rows > 0) {
			...
			$queryx=$this->con->query("select * from barang");
			...
			$id_urut_kode = 'LSN'.$id_kategori.'-'.$a;
			$query2=$this->con->query("insert into barang set kode='$id_urut_kode', nama_barang='$nama_barang',id_kategori='$id_kategori',stok='$stok',harga_beli='$harga_beli',harga_jual='$harga_jual'");
			if ($query2===TRUE) {
	...
	function tampil_barang($keyword){
		//echo $keyword; die;
		if ($keyword=="null") {
			$query=$this->con->query("select barang.id_barang,barang.kode,barang.nama_barang,barang.stok,barang.harga_beli,barang.harga_jual,barang.date_added,kategori.nama_kategori from barang inner join kategori on kategori.id_kategori=barang.id_kategori");
		}else{
			$query=$this->con->query("select barang.id_barang,barang.kode,barang.nama_barang,barang.stok,barang.harga_beli,barang.harga_jual,barang.date_added,kategori.nama_kategori from barang inner join kategori on kategori.id_kategori=barang.id_kategori where nama_barang like '%$keyword%'");
		}
		if ($query->num_rows > 0) {
			$no=1;
	...
	function tampil_barang_filter($id_cat){
			$query=$this->con->query("select barang.kode,barang.id_barang,barang.nama_barang,barang.stok,barang.harga_beli,barang.harga_jual,barang.date_added,kategori.nama_kategori from barang inner join kategori on kategori.id_kategori=barang.id_kategori where kategori.id_kategori='$id_cat'");
		if ($query->num_rows > 0) {
	...
	function show_jumlah_trans_date(){
		$date = date("d-m-Y");
		$query=$this->con->query("select * from transaksi where tgl_transaksi='".$date."'");
		echo $query->num_rows;
	}
	...
	// custom
	function tambah_tempo($id_barang,$jumlah,$diskon,$trx){
		...
		if ($data['stok'] < $jumlah) {
			...
			if ($q->num_rows > 0) {
				...
				$total_sebelum_diskon=$jumbel*$data['harga_jual'];
				$a = $data['harga_jual']*($diskon/100);
				$harga_stl_diskon = $data['harga_jual'] - $a;
				$total_harga=$jumbel*$harga_stl_diskon;
				$dbquery=$this->con->query("update tempo set jumlah_beli='$jumbel',total_harga='$total_harga',total_sebelum_diskon='$total_sebelum_diskon',diskon='$diskon' where id_barang='$id_barang'");
					if ($dbquery === TRUE) {
				...
				$total_sebelum_diskon=$jumlah*$data['harga_jual'];
				$a = $data['harga_jual']*($diskon/100);
				$harga_stl_diskon = $data['harga_jual'] - $a;
				$total_harga=$jumlah*$harga_stl_diskon;
				$query1=$this->con->query("insert into tempo set id_barang='$id_barang',jumlah_beli='$jumlah',diskon='$diskon',total_harga='$total_harga',total_sebelum_diskon='$total_sebelum_diskon',trx='$trx'");
				if ($query1 === TRUE) {
					$this->con->query("update barang set stok=stok-$jumlah where id_barang='$id_barang'");
					$this->alert("Tersimpan");
					$this->redirect("transaksi.php?action=transaksi_baru");
