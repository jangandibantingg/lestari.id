...
if (isset($_GET['action'])) {
	...
	if ($action=="tambah_tempo") {
		$root->tambah_tempo($_POST['id_barang'],$_POST['jumlah'],$_POST['diskon'],$_POST['trx']);
	}
	...
	if ($action=="selesai_transaksi") {
		...
		while ($f=$query2->fetch_assoc()) {
			$root->con->query("insert into sub_transaksi set id_barang='$f[id_barang]',id_transaksi='$id_transaksi2',jumlah_beli='$f[jumlah_beli]',total_harga='$f[total_harga]',diskon='$f[diskon]',total_sebelum_diskon='$f[total_sebelum_diskon]',no_invoice='$trx'");
		}
